...
def sam_segment(
    ...
    input_point = np.nonzero(input_point)
    input_point = np.array(list(zip(input_point[1], input_point[0])))
    input_point = input_point[np.random.choice(input_point.shape[0], 1000),:]
...
class SAMSimilarity:
    ...
    def main(self, sam_model, image, prediction_image):
        ...
        for i in range(len(image)):
            ...
            item = item.cpu().to(torch.float16).numpy()
